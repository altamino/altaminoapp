...
class TwilightManager {
    ...
    private void updateState(Location location) {
        ...
        twilightCalculator.calculateTwilight(jCurrentTimeMillis - DateUtils.ONE_DAY, location.getLatitude(), location.getLongitude());
        ...
        boolean z2 = z;
        twilightCalculator.calculateTwilight(DateUtils.ONE_DAY + jCurrentTimeMillis, location.getLatitude(), location.getLongitude());
        long j5 = twilightCalculator.sunrise;
        if (j3 == -1 || j4 == -1) {
            j = 43200000 + jCurrentTimeMillis;
        } else {
            j = (jCurrentTimeMillis > j4 ? 0 + j5 : jCurrentTimeMillis > j3 ? 0 + j4 : 0 + j3) + DefaultLoadErrorHandlingPolicy.DEFAULT_TRACK_BLACKLIST_MS;
        }
