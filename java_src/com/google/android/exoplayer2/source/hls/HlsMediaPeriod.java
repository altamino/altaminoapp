    ...
    */
    ...
    private void buildAndPrepareSampleStreamWrappers(long j) {
        ...
        while (i < list2.size()) {
            ...
            hlsSampleStreamWrapperBuildSampleStreamWrapper.prepareWithMasterPlaylistInfo(new TrackGroupArray(new TrackGroup(rendition.format)), 0, TrackGroupArray.EMPTY);
            i = i2 + 1;
        }
    ...
    */
    ...
    private void buildAndPrepareAudioSampleStreamWrappers(long j, List<HlsMasterPlaylist.Rendition> list, List<HlsSampleStreamWrapper> list2, List<int[]> list3, Map<String, DrmInitData> map) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (hashSet.add(str)) {
                ...
                HlsSampleStreamWrapper hlsSampleStreamWrapperBuildSampleStreamWrapper = buildSampleStreamWrapper(1, (Uri[]) arrayList.toArray(new Uri[0]), (Format[]) arrayList2.toArray(new Format[0]), null, Collections.emptyList(), map, j);
                ...
                if (this.allowChunklessPreparation && z) {
                    hlsSampleStreamWrapperBuildSampleStreamWrapper.prepareWithMasterPlaylistInfo(new TrackGroupArray(new TrackGroup((Format[]) arrayList2.toArray(new Format[0]))), 0, TrackGroupArray.EMPTY);
                }
            }
        }
    }
    ...
    private static Map<String, DrmInitData> deriveOverridingDrmInitData(List<DrmInitData> list) {
        ...
        while (i < arrayList.size()) {
            ...
            i++;
            DrmInitData drmInitDataMerge = drmInitData;
            int i2 = i;
            while (i2 < arrayList.size()) {
                DrmInitData drmInitData2 = (DrmInitData) arrayList.get(i2);
                if (TextUtils.equals(drmInitData2.schemeType, str)) {
                    drmInitDataMerge = drmInitDataMerge.merge(drmInitData2);
                    arrayList.remove(i2);
                } else {
                    i2++;
                }
