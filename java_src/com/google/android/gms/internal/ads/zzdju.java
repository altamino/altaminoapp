...
abstract class zzdju implements zzdlk {
    ...
    zzdju(byte[] bArr, int i) throws InvalidKeyException {
        if (bArr.length != 32) {
    ...
    public final byte[] zzo(byte[] bArr) throws GeneralSecurityException {
        if (bArr.length > Integer.MAX_VALUE - zzauv()) {
            throw new GeneralSecurityException("plaintext too long");
        }
        ByteBuffer byteBufferAllocate = ByteBuffer.allocate(zzauv() + bArr.length);
        zza(byteBufferAllocate, bArr);
        return byteBufferAllocate.array();
    }

    final void zza(ByteBuffer byteBuffer, byte[] bArr) throws GeneralSecurityException {
        if (byteBuffer.remaining() - zzauv() < bArr.length) {
            throw new IllegalArgumentException("Given ByteBuffer output is too small");
        }
        byte[] bArrZzff = zzdlo.zzff(zzauv());
        byteBuffer.put(bArrZzff);
        ByteBuffer byteBufferWrap = ByteBuffer.wrap(bArr);
        int iRemaining = byteBufferWrap.remaining();
        int i = (iRemaining / 64) + 1;
        for (int i2 = 0; i2 < i; i2++) {
            ByteBuffer byteBufferZzc = zzc(bArrZzff, this.zzgyg + i2);
            if (i2 == i - 1) {
                zzdjs.zza(byteBuffer, byteBufferWrap, byteBufferZzc, iRemaining % 64);
            } else {
                zzdjs.zza(byteBuffer, byteBufferWrap, byteBufferZzc, 64);
            }
        }
    }

    final ByteBuffer zzc(byte[] bArr, int i) {
        int[] iArrZzb = zzb(zzr(bArr), i);
        int[] iArr = (int[]) iArrZzb.clone();
        zzc(iArr);
        for (int i2 = 0; i2 < iArrZzb.length; i2++) {
    ...
    static void zza(int[] iArr, int[] iArr2) {
        ...
        System.arraycopy(iArr2, 0, iArr, zzgye.length, 8);
    }
