...
public final class zzdkr {
    ...
    public static ECParameterSpec zza(zzdkt zzdktVar) throws NoSuchAlgorithmException {
        int i = zzdks.zzgzz[zzdktVar.ordinal()];
        if (i == 1) {
    ...
    public static ECPublicKey zza(zzdkt zzdktVar, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ECParameterSpec eCParameterSpecZza = zza(zzdktVar);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        zza(eCPoint, eCParameterSpecZza.getCurve());
        return (ECPublicKey) zzdkx.zzhav.zzgt("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCParameterSpecZza));
    }

    public static ECPrivateKey zza(zzdkt zzdktVar, byte[] bArr) throws GeneralSecurityException {
        return (ECPrivateKey) zzdkx.zzhav.zzgt("EC").generatePrivate(new ECPrivateKeySpec(new BigInteger(1, bArr), zza(zzdktVar)));
    }

    public static KeyPair zza(ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        KeyPairGenerator keyPairGeneratorZzgt = zzdkx.zzhau.zzgt("EC");
        keyPairGeneratorZzgt.initialize(eCParameterSpec);
        return keyPairGeneratorZzgt.generateKeyPair();
    }

    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) throws IllegalStateException, GeneralSecurityException {
        ...
        KeyAgreement keyAgreementZzgt = zzdkx.zzhat.zzgt("ECDH");
        keyAgreementZzgt.init(eCPrivateKey);
        try {
            keyAgreementZzgt.doPhase(publicKeyGeneratePublic, true);
            byte[] bArrGenerateSecret = keyAgreementZzgt.generateSecret();
