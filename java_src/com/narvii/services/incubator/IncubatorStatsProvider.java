...
public class IncubatorStatsProvider implements AutostartServiceProvider<StatsService> {
    StatsService stats;
    ...
    public void resume(NVContext nVContext, StatsService statsService) {
        if (nVContext instanceof NVActivity) {
            statsService.touchOrResume(IncubatorApplication.getCommunityId(nVContext));
    ...
    public void pause(NVContext nVContext, StatsService statsService) {
        if (nVContext instanceof NVActivity) {
            statsService.pause(IncubatorApplication.getCommunityId(nVContext));
        } else if (nVContext instanceof Application) {
            statsService.stop();
        }
    }
