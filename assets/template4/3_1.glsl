...
void main() {
    ...
    if (uv2.x >= dx && uv2.x <= 1. - dx && uv2.y >= dy && uv2.y <= 1. - dy) {
        ...
        uv2.x -= process * 2.;
        uv2 = stretchUV(uv2, process);
        gl_FragColor = (process < stage1) ? motionBlur(inputImageTexture, uv2, vec2(speed, 0.)) : motionBlur(inputImageTexture2, uv2, vec2(speed, 0.));
    } else {
        uv = uv - 0.5;
        uv = uv / (1. + 1.8 * sin(process * PI)) + 0.5;
//        uv = fieldView(uv, process);
        gl_FragColor = (process < stage1) ? gaussianBlur_X(inputImageTexture, uv) : gaussianBlur_X(inputImageTexture2, uv);
    }
}
