...
float easeQuintInOut(mediump float t) {
...
void main() {
    ...
    float division = 0.32;
    float t = easeQuintInOut(f / d);
    float speed = sin(t * PI) * 0.5;
    vec4 color = vec4(0.);

    if (t < division) {
        uv.x += t * 6.;
        uv = stretchUV(uv, t);
        color += motionBlur(inputImageTexture, uv, vec2(speed, 0.));
        //gl_FragColor = vec4(color.rgb * pow(2.0, (speed * 0.5)), 1.0);
    } else {
        uv2.x += t * 6.;
        uv2 = stretchUV(uv2, t);
        color += motionBlur(inputImageTexture2, uv2, vec2(speed, 0.));
        //gl_FragColor = vec4(color.rgb * pow(2.0, (speed * 0.5)), 1.0);
    }
