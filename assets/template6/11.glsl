...
void main() {
    ...
    float t = (f <= stage1) ? easeCubicOut(f / stage1) : easeCubicOut((f - stage1) / (division - stage1));
    ...
    if (f <= stage1) {
        float blur = 0.3;
        float rot1 = (t <= 0.5) ? rotation * t : -rotation + rotation * t;
        float rot2 = (t <= 0.5) ? rotation * (t + blur * t) : -rotation + rotation * (t + blur * (1.0 - t));
        vec2 size = (t <= 0.5) ? inputSize : inputSize2;
        vec2 uvs = (t <= 0.5) ? uv : uv2;
        uvs = (uvs - vec2(0.5)) * size;
        uvs = mat2(cos(rot1), -sin(rot1), sin(rot1), cos(rot1)) * uvs;
        uvs = uvs / size + vec2(0.5);
        vec2 uvs1 = (t <= 0.5) ? uv : uv2;
        uvs1 = (uvs1 - vec2(0.5)) * size;
        uvs1 = mat2(cos(rot2), -sin(rot2), sin(rot2), cos(rot2)) * uvs1;
        uvs1 = uvs1 / size + vec2(0.5);
        vec2 speed = uvs1 - uvs;
        gl_FragColor = (t <= 0.5) ? motionBlur(inputImageTexture, uvs, speed, t) : motionBlur(inputImageTexture2, uvs, speed, t);
    } else {
