...
vec4 textureColor(float divider, float sectionIndex, float progress, float blurProgress, vec2 uvBase, vec2 uvOverlay) {
    ...
    if (textureCoordinate.y <= divider * (sectionIndex - 1.)) {
        ...
        vec2 speed = vec2(pow(cos(blurProgress * PI_TWO), 1.2), 0.) * 0.4;
        vec4 overlay = motionBlur(inputImageTexture2, uvOverlay, speed, pow(blurProgress, 1.5));
...
void main() {
    ...
    float progress = easeElasticOut((f - section * (sectionIndex - 1.)) / section);
    float blurProgress = easeExpoOut((f - section * (sectionIndex - 1.)) / section);
    gl_FragColor = textureColor(divider, sectionIndex, progress, blurProgress, uv, uv2);
}